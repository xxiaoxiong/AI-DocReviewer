...
"""
    ...
    def _build_review_prompt_v2_cot(
        self,
        text: str,
        relevant_rules: List[Dict[str, Any]],
        context: Optional[str] = None
        ...
        """
        构造审核 prompt（思维链版本 - 用于复杂场景）
        
        使用 Chain of Thought 引导模型逐步推理
        """
        top_rules = relevant_rules[:2] if len(relevant_rules) > 2 else relevant_rules
        
        prompt = f"""你是公文审核专家。请按以下步骤审核文本：
...
"""
        ...
        for i, rule in enumerate(top_rules, 1):
            prompt += f"{i}. {rule.get('description', '')} (规则ID: {rule.get('rule_id', '')})\n"
            if rule.get('positive_examples'):
                prompt += f"   正确示例: {rule.get('positive_examples', [])[0]}\n"
            if rule.get('negative_examples'):
                prompt += f"   错误示例: {rule.get('negative_examples', [])[0]}\n"
...
以JSON格式输出，格式：
{
  "reasoning": "简要说明判断过程（可选）",
  "issues": [
    {
      "position": "位置",
      "rule_id": "规则ID",
      "category": "类别",
      "original_text": "原文（不超过30字）",
